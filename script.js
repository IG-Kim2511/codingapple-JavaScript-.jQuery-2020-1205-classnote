...
// 🦄c42 for반복문. i.let.function parameter (한글자씩 출현하는 Typewriting 애니메이션 만들기 2)
...
var h1태그42 = document.querySelector('.c42 h1');
var 원래글씨42 = document.querySelector('.c42 h1').innerHTML;

document.querySelector('.c42 button').addEventListener('click', function(){

  h1태그42.innerHTML = ''; 

  for (let i = 0; i < 원래글씨42.length; i++){  /*  2) 6) */
    setTimeout(function(){ 
      h1태그42.innerHTML = h1태그42.innerHTML + 원래글씨42[i] /* 4-2) */
    }, i * 500);  /* 4-3) */
  }
})
...
8) parameter활용 : 
함수화하실 땐 변수들을 파라미터로 입력한다고 정의해주시는게 매우 좋은 관습이자 옳은 방법입니다. 
(그리고 나중에 재활용하실 땐 h1태그, 원래글씨라는 변수들 내용이 달라질 수도 있으니까요)
*/

var h1태그42_2 = document.querySelector('.c42_2 h1');
var 원래글씨42_2 = document.querySelector('.c42_2 h1').innerHTML;

document.querySelector('.c42_2 button').addEventListener('click', function(){
  애니메이션(h1태그42_2, 원래글씨42_2)  /*7-3) 함수 선언  8) */
})

function 애니메이션(h1태그42_2, 원래글씨42_2){  /*7-2) 함수화  8)  */
  h1태그42_2.innerHTML = '';
  for (let i = 0; i < 원래글씨42_2.length; i++){
    setTimeout(function(){ 
      h1태그42_2.innerHTML = h1태그42_2.innerHTML + 원래글씨42_2[i]
    }, i * 500);
  }
}